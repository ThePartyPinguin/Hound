...
bool input_system::is_key_down(key_code code)
{
	if (!m_input_info_.count(code))
		return false;

	return m_input_info_[code].action != key_action::RELEASED;
}

bool input_system::is_key_pressed(key_code code)
{
	if (!m_input_info_.count(code))
		return false;

	return m_input_info_[code].action != key_action::RELEASED && m_input_info_[code].frame == m_current_frame_;
}

bool input_system::is_key_released(key_code code)
{
	if (!m_input_info_.count(code))
		return false;
...
void input_system::process_input_events()
{
	++m_current_frame_;

	auto it = m_input_info_.begin();
	while(it != m_input_info_.end())
	{
		if(it->second.action == key_action::RELEASED)
		{
			it = m_input_info_.erase(it);
		}
		else
		{
			++it;
...
{
	m_input_info_[e.get_button()] = { e.get_button(), e.get_action(), e.get_mods(), m_current_frame_ };
}

void input_system::on_event(const window_mouse_move_event& e)
{
	m_mouse_data_.window_mouse_position = e.get_mouse_pos();

	const vec2_i window_origin = e.get_window_object()->get_rect().get_origin();
	const vec2_d global_mouse_pos = e.get_mouse_pos() + vec2_d(window_origin.get_x(), window_origin.get_y());
